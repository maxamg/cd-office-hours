...
1. To execute the playbook:
   ...
   ansible-playbook -vvv -i environments/dev/inventory --user vagrant --private-key=~/.vagrant.d/insecure_private_key deploy_java_app.yml
   ```

   Vagrant >= 1.7
   ```
   vagrant up --provider virtualbox
   ansible-playbook -vvv -i environments/dev/inventory --user vagrant --private-key=.vagrant/machines/default/virtualbox/private_key deploy_java_app.yml
   ```
